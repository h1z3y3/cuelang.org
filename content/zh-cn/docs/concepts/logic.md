...
title = "CUE 的逻辑"
weight = 1
description = "CUE 的理论基础以及什么让 CUE 与众不同"
mermaid = true
+++

这个页面会解释 CUE 所有的核心概念，虽然对于学习这门语言不是必需的，但是能帮助自上而下的理解以及作为参考的框架。
...
CUE 与常见编程语言或其他配置语言不同主要有两个方面：

- 类型 _是_ 值
- 值 （以及类型）排列成 {{< glossary_tooltip term_id="lattice" >}}

这些性质几乎与 CUE 的方方面面都密切相关。
它们简化了语言，而在其他语言中很多概念都是截然不同的。
不管对于人或者机器，由此产生的顺序独立性简化了 值 的推理。

它还强制要求语言形式保持严谨，比如要准确定义是否可选、设置默认值或者 null，
以确保所有的值都在能在一个 `值格` 中，从而不会回旋的空间。

最终，这一切的结合得以实现很多独特的功能，
比如：

- 仅用一门语言来定义数据、Schema、进行校验以及策略约束
- 元推理（meta reasoning），比如用于确定一个 Schema 的新版本是否向后兼容
- 自动改写（automated rewriting），就像 `cue trim` 做的那样
- 创建多源（multi-source）的约束管道，保留跨标准化的文档

等等。

## {{< glossary_tooltip term_id="lattice" >}}

CUE 中的所有 值，包括大多数编程语言中的被认为的 类型，
都是在某个单一层次结构中部分有序的（准确的说，在 `格` 中）。
甚至整个配置和 Schema 都被放在这个层次中。

### 什么是格？

{{< alert >}}
这部分内容有助于理解是`格`，但是对于掌握后面的章节并不是必需的，也不是 CUE 本身相关的内容。
可以按照自己意愿跳过这一部分。
{{< /alert >}}

`格` 是一个部分有序的集合，对于任意两个元素都有一个唯一的最小上界（并）和最大下界（交）。
根据定义，这意味着总会有一个单一的根节点（顶）和一个单一的叶子节点（底）。

让我们通过例子来看是什么意思，下面的图分别展示了 2 个元素和 3 个元素的集合，根据子集排序，从而组成格的所有值。
...
如果 元素B 是 元素A 的子集，就会有从 A 指向 B 的路径。
更简单的，我们可以说 A _包含_ B，或者 B 是 A 的一个 _实例_。
在我们的例子中，因为我们用子集关系来定义格，所以 `{x}` 是 `{x, y}` 的一个实例。
但是我们可以使用任何我们想要的关系来定义格，只要格的性质是成立的。

// TODO: make it correct and smooth

格有一个要点是对于任意两个元素，这两个元素都有一个 _唯一_ 的实例，
而这个实例包含了这两个元素的所有实例。
这被称为最大下确界，或者 交运算。
